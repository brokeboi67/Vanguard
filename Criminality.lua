...
-- ── CRIM SHOT TRACERS (GNX_S FireServer: origin + {dir…} — dedicated, not Visuals→ShotTracers) ──
-- Cobalt: Events.GNX_S:FireServer(time, token, Tool, id, Vector3 origin, { Vector3 dir }, bool)
misc.shotTracers = {
	conns = {},
	active = false,
	folder = nil,
...
function misc.shotTracers.collectArgs(outVec, outDir, outPlr, outTool, outNum, depth, v, asDir)
	if depth > 4 or v == nil then
		return
	end
	local t = typeof(v)
	if t == "Vector3" then
		-- Unit-ish vectors are aim dirs (GNX_S packs dirs in a table); world positions are large
		local mag = v.Magnitude
		if asDir or (mag > 0.05 and mag < 1.75) then
			outDir[#outDir + 1] = v
		else
			outVec[#outVec + 1] = v
		end
	...
	elseif t == "table" then
		...
		for i = 1, #keys do
			...
			if x ~= nil then
				local key = keys[i]
				local dirKey = key == "Direction" or key == "direction" or key == "Dir" or key == "dir"
					or key == "Look" or key == "look" or key == "Normal" or key == "normal"
				misc.shotTracers.collectArgs(outVec, outDir, outPlr, outTool, outNum, depth + 1, x, dirKey)
			end
		end
		-- Array tables in GNX_S are direction lists: { Vector3.new(...) }
		local arrayOnly = true
		local any = false
		...
		for k, x in pairs(v) do
			if typeof(k) == "number" then
				misc.shotTracers.collectArgs(outVec, outDir, outPlr, outTool, outNum, depth + 1, x, arrayOnly and any)
			end
		end
	end
end
...
function misc.shotTracers.parseArgs(...)
	local vecs, dirs, plrs, tools, nums = {}, {}, {}, {}, {}
	local n = select("#", ...)
	for i = 1, n do
		misc.shotTracers.collectArgs(vecs, dirs, plrs, tools, nums, 0, select(i, ...), false)
	end
	-- Packed floats: x,y,z,x,y,z → Vector3s (common Crim remote style)
	if #vecs == 0 and #nums >= 6 then
		for i = 1, #nums - 2, 3 do
			local cand = Vector3.new(nums[i], nums[i + 1], nums[i + 2])
			if cand.Magnitude > 0.05 and cand.Magnitude < 1.75 then
				dirs[#dirs + 1] = cand
			else
				vecs[#vecs + 1] = cand
			end
		end
	elseif #vecs == 1 and #nums >= 3 and #dirs == 0 then
		dirs[#dirs + 1] = Vector3.new(nums[1], nums[2], nums[3])
	end
	return vecs, dirs, plrs, tools, nums, n
end

function misc.shotTracers.toolIsLocal(tool)
	local lp = getLP()
	if not lp or not tool then
		return false
	end
	if tool:IsDescendantOf(lp) then
		return true
	end
...
function misc.shotTracers.localMuzzle()
	...
	-- Prefer ViewModel gun tip
	if cam then
		local vm = cam:FindFirstChild("ViewModel")
		if vm then
			local wh = vm:FindFirstChild("WeaponHandle", true) or vm:FindFirstChild("Muzzle", true)
			if wh and wh:IsA("BasePart") then
				return wh.Position, cam.CFrame.LookVector
			end
			local tip = vm:FindFirstChildWhichIsA("MeshPart", true)
			if tip then
				return tip.Position, cam.CFrame.LookVector
			end
		end
	end
	if char then
		...
		if tool then
			local wh = tool:FindFirstChild("WeaponHandle", true) or tool:FindFirstChild("Muzzle", true)
			if wh and wh:IsA("BasePart") then
				local look = cam and cam.CFrame.LookVector or wh.CFrame.LookVector
				return wh.Position, look
			end
		end
		local head = char:FindFirstChild("Head")
		if head and cam then
			return head.Position, cam.CFrame.LookVector
		end
	end
	if cam then
...
function misc.shotTracers.drawOwnFromAim()
	local S = _G.__VG_S
	if not S or not S.CrimShotTracers or S.CrimShotTracersSelf == false then
		return
	end
	local now = tick()
	if now - (misc.shotTracers._lastOwnAt or 0) < 0.045 then
		return
	end
	misc.shotTracers._lastOwnAt = now
	local from, dir = misc.shotTracers.localMuzzle()
	if typeof(from) ~= "Vector3" or typeof(dir) ~= "Vector3" or dir.Magnitude < 0.01 then
		return
	end
	dir = dir.Unit
...
function misc.shotTracers.onTrailPart(inst)
	...
	local folder = misc.shotTracers.folder
	if folder and inst:IsDescendantOf(folder) then
		return
	end
	local n = string.lower(inst.Name)
	if n ~= "bullettrail" and n ~= "tracer" and n ~= "bullet_trail" and string.find(n, "trail", 1, true) == nil then
		return
	end
	-- Ignore tiny debris
	if inst.Size.Magnitude < 1.2 and inst.Size.Z < 1.2 then
		return
	end
	task.defer(function()
		...
		local len = math.max(inst.Size.Z, inst.Size.Y, inst.Size.X)
		if len < 1.5 then
			-- Point trail — stretch along look
			len = 8
		end
		local from = cf.Position - cf.LookVector * (len * 0.5)
		local to = cf.Position + cf.LookVector * (len * 0.5)
		local lp = getLP()
		local isOwn = false
		if lp and lp.Character then
			local hrp = lp.Character:FindFirstChild("HumanoidRootPart")
			if hrp and (from - hrp.Position).Magnitude < 18 then
				isOwn = true
			end
		end
...
function misc.shotTracers.holdingGun()
	local lp = getLP()
	local char = lp and lp.Character
	if not char then
		return false
	...
	-- Crim melees (Chainsaw, Bat…) also have WeaponHandle — skip those
	if tool:FindFirstChild("Melee") then
		return false
	end
	return tool:FindFirstChild("WeaponHandle", true) ~= nil
		or tool:FindFirstChild("Gun") ~= nil
		or tool:FindFirstChild("MagPart", true) ~= nil
end
...
function misc.shotTracers.matchPending(hitPos)
	local list = misc.shotTracers.pending
	local now = tick()
	local best, bestScore, bestIdx = nil, -1, nil
	for i = 1, #list do
		local p = list[i]
		if now - p.t <= 0.8 then
			local score = 0
...
function misc.shotTracers.onShoot(...)
	...
	misc.shotTracers.maybeLog("GNX_S", ...)
	local vecs, dirs, plrs, tools = misc.shotTracers.parseArgs(...)
	local isOwn = misc.shotTracers.isOwnShot(plrs)
	if not isOwn then
		for i = 1, #tools do
			if misc.shotTracers.toolIsLocal(tools[i]) then
				isOwn = true
				break
			end
		end
	end
	-- FireServer from this client has no Player arg — treat as own
	if #plrs == 0 and (#tools == 0 or isOwn) then
		isOwn = true
	end
	-- GNX_S primary shape: origin + one or more dirs
	if #vecs >= 1 and #dirs >= 1 then
		local from = vecs[1]
		if #plrs >= 1 then
			local m = misc.shotTracers.muzzleFromPlayer(plrs[1])
			if m and (from - m).Magnitude > 80 then
				-- Prefer player muzzle if remote origin is nonsense
				from = m
			end
		end
		for i = 1, #dirs do
...
function misc.shotTracers.onHit(...)
	...
	misc.shotTracers.maybeLog("GNX_H", ...)
	local vecs, dirs, plrs, tools = misc.shotTracers.parseArgs(...)
	local isOwn = misc.shotTracers.isOwnShot(plrs)
	if not isOwn then
		for i = 1, #tools do
	...
	elseif isOwn or #plrs == 0 then
		local m = misc.shotTracers.localMuzzle()
		if m then
			misc.shotTracers.draw(m, hit, true)
		end
	end
end
...
function misc.shotTracers.start()
	...
	misc.shotTracers.logLeft = 12

	local events = RepSt:FindFirstChild("Events")
	if events then
		-- Criminality live shoot remote (Cobalt)
		misc.shotTracers.bindRemote(events:FindFirstChild("GNX_S"), misc.shotTracers.onShoot)
		misc.shotTracers.bindRemote(events:FindFirstChild("GNX_H"), misc.shotTracers.onHit)
		misc.shotTracers.bindRemote(events:FindFirstChild("GNX_U"), misc.shotTracers.onShoot)
		...
		misc.shotTracers.bindRemote(events:FindFirstChild("GZ_S"), misc.shotTracers.onShoot)
		misc.shotTracers.bindRemote(events:FindFirstChild("GZ_H"), misc.shotTracers.onHit)
		misc.shotTracers.bindRemote(events:FindFirstChild("GZ_U"), misc.shotTracers.onShoot)
	end

	-- Game BulletTrail clones (other players when remotes are opaque)
	table.insert(
		misc.shotTracers.conns,
		workspace.DescendantAdded:Connect(function(inst)
			misc.shotTracers.onTrailPart(inst)
		end)
	)

	-- FireServer namecall — GNX_S is client→server (own shots land here)
	...
	then
		...
		local function isGunRemote(self)
			...
			if SHOT_REMOTE_NAMES[nm] then
				return true
			end
			local l = string.lower(nm)
			return string.find(l, "shoot", 1, true) ~= nil
				or string.find(l, "gun_", 1, true) ~= nil
				or string.sub(l, 1, 4) == "gnx_"
				or l == "gz_s"
				or l == "gz_h"
		end
		local function handler(self, ...)
			local method = getnamecallmethod()
			if (method == "FireServer" or method == "InvokeServer") and isGunRemote(self) then
				local nm = self.Name
				misc.shotTracers.maybeLog("FS:" .. tostring(nm), ...)
				if nm == "Gun_Hit" or nm == "GZ_H" or nm == "GNX_H" then
	...
	pcall(function()
		if typeof(_G.__VG_LOG_FILE) == "function" then
			_G.__VG_LOG_FILE("INFO", "[VG:shotTrace] active: GNX_S FireServer + BulletTrail")
		end
	end)
	print("[VG:shotTrace] active: GNX_S FireServer + BulletTrail")
end
...
-- Optional MeshVariant map: gun → display → SkinVariants model name (e.g. bat_cashcane)
function misc.skinChanger.getTexCatalog()
	...
	local function ingestGunBucket(gun, skins)
		if typeof(gun) ~= "string" or typeof(skins) ~= "table" then
			return
		end
		cat[gun] = cat[gun] or {}
		meshCat[gun] = meshCat[gun] or {}
		for disp, tex in pairs(skins) do
			if typeof(disp) == "string" then
				if typeof(tex) == "number" then
					cat[gun][disp] = tex
				elseif typeof(tex) == "table" then
					local id = tonumber(tex.tex or tex.TextureID or tex.id or tex[1])
					local mesh = tex.mesh or tex.MeshVariant or tex.meshVariant or tex[2]
					if id then
						cat[gun][disp] = id
					end
					if typeof(mesh) == "string" and mesh ~= "" then
						meshCat[gun][disp] = mesh
					end
				elseif typeof(tex) == "string" and tonumber(tex) then
					cat[gun][disp] = tonumber(tex)
				end
			end
		end
	end
	if typeof(_G.__VG_CrimSkinIds) == "table" then
		for gun, skins in pairs(_G.__VG_CrimSkinIds) do
			ingestGunBucket(gun, skins)
		end
	end
	if typeof(_G.__VG_CrimSkinMeshes) == "table" then
		for gun, skins in pairs(_G.__VG_CrimSkinMeshes) do
			if typeof(skins) == "table" then
				meshCat[gun] = meshCat[gun] or {}
				for disp, mesh in pairs(skins) do
					if typeof(disp) == "string" and typeof(mesh) == "string" and mesh ~= "" then
						meshCat[gun][disp] = mesh
					end
				end
			end
	...
	pcall(function()
		...
		for _, path in ipairs({
			"VG_CrimSkinMeshes.json",
			"CrimSkinMeshes.json",
			"Vanguard/CrimSkinMeshes.json",
			"VG_FullDump/12_gc_skin_meshes.json",
		}) do
			if isfile(path) then
				local ok, decoded = pcall(function()
					return HS:JSONDecode(readfile(path))
	...
	-- Live getgc fill for MeshVariant + Rarity only
	task.defer(function()
...
function misc.skinChanger.getMeshVariantCatalog()
	misc.skinChanger.getTexCatalog() -- ensure load + deferred harvest kicked
			...
			if typeof(v) == "table" then
				...
				then
					...
					if typeof(tex) == "number" and tex > 1000 then
						...
						if cat then
							cat[name] = cat[name] or {}
							if not cat[name][disp] then
								cat[name][disp] = tex
							end
						end
					end
...
function misc.skinChanger.lookupMeshVariant(gunName, displayName)
	...
	local meshCat = misc.skinChanger.getMeshVariantCatalog()
	local want = string.lower(displayName)
	local wantC = string.gsub(want, "%s+", "")
	for _, key in ipairs(misc.skinChanger.catalogKeysForGun(gunName)) do
		local bucket = meshCat[key]
		if typeof(bucket) == "table" then
			local direct = bucket[displayName]
			if typeof(direct) == "string" and direct ~= "" then
				return direct
			end
			for disp, mesh in pairs(bucket) do
				if typeof(disp) == "string" and typeof(mesh) == "string" then
					local d = string.lower(disp)
					if d == want or string.gsub(d, "%s+", "") == wantC then
						return mesh
					end
...
function misc.skinChanger.lookupMeshVariantByTexId(gunName, texId)
	...
	for _, key in ipairs(misc.skinChanger.catalogKeysForGun(gunName)) do
		...
		if typeof(bucket) == "table" then
			for disp, tex in pairs(bucket) do
				if tonumber(tex) == texId then
					return misc.skinChanger.lookupMeshVariant(gunName, disp)
				end
			end
		end
	end
...
-- MeshVariant string → V:Guns/mare_trickshot (find under SkinVariants)
function misc.skinChanger.meshVariantToSkinKey(meshVar)
	if typeof(meshVar) ~= "string" or meshVar == "" then
		return nil
	end
	if string.sub(meshVar, 1, 2) == "V:" then
		return meshVar
	end
	local slash = string.find(meshVar, "/", 1, true)
	if slash then
		return "V:" .. meshVar
	end
	local root = misc.skinChanger.getSkinVariants()
	if root then
		for _, cat in ipairs({ "Melees", "Guns", "Charms" }) do
			local folder = root:FindFirstChild(cat)
			local m = folder and folder:FindFirstChild(meshVar)
			if m and m:IsA("Model") then
				return misc.skinChanger.variantKey(cat, meshVar)
			end
		end
	end
	-- guess Melees vs Guns from slug map
	local g = misc.skinChanger.parseVariantModelName(meshVar)
	local meleeSlugs = {
	...
	local cat = meleeSlugs[slug] and "Melees" or "Guns"
	return misc.skinChanger.variantKey(cat, meshVar)
end
...
-- Crim silencer / special tools: Tommy-S, M4A1-X = same skin set as base
function misc.skinChanger.baseGunName(gunName)
	...
	if #n > 2 and string.sub(n, -2) == "-X" then
		n = string.sub(n, 1, #n - 2)
	end
	return n
end
...
function misc.skinChanger.getSkinVariants()
	local storage = RepSt:FindFirstChild("Storage")
	return storage and storage:FindFirstChild("SkinVariants")
end

function misc.skinChanger.getDisplayWepModels()
	local storage = RepSt:FindFirstChild("Storage")
	return storage and storage:FindFirstChild("DisplayWepModels")
end

-- One-shot wipe of leftover VG upgrade junk from older builds (feature removed).
function misc.skinChanger.cleanupLeftoverUpgradeJunk()
	if misc.skinChanger._upgJunkCleaned then
		return
	end
	misc.skinChanger._upgJunkCleaned = true
	local function wipe(root)
		...
		for _, d in ipairs(root:GetDescendants()) do
			...
			elseif d:IsA("BasePart") and d:GetAttribute("VG_UpgHidden") == true then
				pcall(function()
					local prev = d:GetAttribute("VG_UpgPrevLTM")
	...
	if lp and lp:FindFirstChild("Backpack") then
		for _, ch in ipairs(lp.Backpack:GetChildren()) do
			if ch:IsA("Tool") then
				wipe(ch)
			end
		end
	...
	local S = _G.__VG_S
	if S then
		S.CrimGunUpgradeLooks = nil
		if typeof(_G.__VG_CONFIG) == "table" and typeof(_G.__VG_CONFIG.SaveGlobals) == "function" then
			pcall(_G.__VG_CONFIG.SaveGlobals, S)
		end
	end
	pcall(function()
		misc.skinChanger.log("cleaned leftover upgrade junk (feature removed)")
	end)
end
function misc.skinChanger.isVariantSkinKey(skinKey)
	return typeof(skinKey) == "string" and string.sub(skinKey, 1, 2) == "V:"
end

function misc.skinChanger.variantKey(category, modelName)
	return "V:" .. tostring(category) .. "/" .. tostring(modelName)
end

function misc.skinChanger.variantPathFromKey(skinKey)
	if not misc.skinChanger.isVariantSkinKey(skinKey) then
		return nil, nil
	end
	local rest = string.sub(skinKey, 3)
	local slash = string.find(rest, "/", 1, true)
	if not slash then
		return nil, rest
	end
	return string.sub(rest, 1, slash - 1), string.sub(rest, slash + 1)
end

function misc.skinChanger.normToken(s)
	return string.lower(string.gsub(tostring(s or ""), "[^%w]", ""))
end

function misc.skinChanger.titleFromSlug(slug)
	if typeof(slug) ~= "string" or slug == "" then
		return "?"
	end
	local parts = {}
	for p in string.gmatch(slug, "[^_]+") do
		if #p > 0 then
			parts[#parts + 1] = string.upper(string.sub(p, 1, 1)) .. string.sub(p, 2)
		end
	end
	if #parts == 0 then
		return slug
...
-- Static slug → gun for SkinVariants folder names (mare_trickshot, sawnoff_eros…)
local VARIANT_SLUG_GUN = {
	mare = "Mare",
	m4a1 = "M4A1",
	sawnoff = "Sawn-Off",
	fireaxe = "Fire-Axe",
...
function misc.skinChanger.parseVariantModelName(modelName)
	if typeof(modelName) ~= "string" or modelName == "" then
		return nil, nil
	end
	local us = string.find(modelName, "_", 1, true)
	local slug = us and string.sub(modelName, 1, us - 1) or modelName
	local rest = us and string.sub(modelName, us + 1) or modelName
	local gun = VARIANT_SLUG_GUN[string.lower(slug)]
	if not gun then
		-- fuzzy: longest catalog/RepPBR gun whose norm matches prefix of compact name
		local compact = misc.skinChanger.normToken(modelName)
		local bestNk, bestGun = "", nil
		local cat = misc.skinChanger.getTexCatalog()
		for g in pairs(cat) do
			local base = misc.skinChanger.baseGunName(g)
			if string.sub(base, -2) == "-1" then
				base = string.sub(base, 1, #base - 2)
			end
			local nk = misc.skinChanger.normToken(base)
			if #nk > #bestNk and string.sub(compact, 1, #nk) == nk then
				bestNk, bestGun = nk, base
			end
		end
		gun = bestGun or slug
		if bestNk ~= "" then
			rest = string.sub(compact, #bestNk + 1)
		end
	end
	return gun, misc.skinChanger.titleFromSlug(rest)
end

function misc.skinChanger.findVariantModel(skinKey)
	local cat, name = misc.skinChanger.variantPathFromKey(skinKey)
	if not name then
		return nil
	end
	local root = misc.skinChanger.getSkinVariants()
	...
	for _, folderName in ipairs({ "Guns", "Melees", "Charms" }) do
...
function misc.skinChanger.previewFromVariant(model)
	if not model then
		return ""
	end
	for _, d in ipairs(model:GetDescendants()) do
		if d:IsA("SurfaceAppearance") then
			local id = misc.skinChanger.contentId(d.ColorMap)
			if id ~= "" then
				return id
			end
		end
	...
	for _, d in ipairs(model:GetDescendants()) do
		if d:IsA("MeshPart") then
			local id = misc.skinChanger.contentId(d.TextureID)
			if id ~= "" then
				return id
			end
		end
...
function misc.skinChanger.listVariantSkinsForGun(gunName)
	local out, seen = {}, {}
	if not gunName then
		return out
	end
	local function addRow(full, label, preview)
		if not full or not label or seen[string.lower(label)] then
			return
		end
		seen[string.lower(label)] = true
		seen[string.gsub(string.lower(label), "%s+", "")] = true
		out[#out + 1] = {
			full = full,
			label = label,
			preview = preview or "",
	...
	-- Official MeshVariant from cosmetic catalog (preferred over folder scrape)
	local meshCat = misc.skinChanger.getMeshVariantCatalog()
	for _, key in ipairs(misc.skinChanger.catalogKeysForGun(gunName)) do
		local bucket = meshCat[key]
		if typeof(bucket) == "table" then
			for disp, meshVar in pairs(bucket) do
				if typeof(disp) == "string" and typeof(meshVar) == "string" and meshVar ~= "" then
					local vkey = misc.skinChanger.meshVariantToSkinKey(meshVar)
					local model = vkey and misc.skinChanger.findVariantModel(vkey)
					addRow(
						vkey,
						disp,
						model and misc.skinChanger.previewFromVariant(model) or ""
					)
				end
			end
		end
	...
	-- Folder scrape for any SkinVariants not yet in catalog
	local root = misc.skinChanger.getSkinVariants()
	if root then
		local base = misc.skinChanger.baseGunName(gunName)
		if string.sub(base, -2) == "-1" then
		...
		local want = misc.skinChanger.normToken(base)
		for _, cat in ipairs({ "Guns", "Melees" }) do
			local folder = root:FindFirstChild(cat)
			if folder then
				for _, ch in ipairs(folder:GetChildren()) do
					if ch:IsA("Model") then
						local g, label = misc.skinChanger.parseVariantModelName(ch.Name)
						if g and misc.skinChanger.normToken(g) == want then
							addRow(
								misc.skinChanger.variantKey(cat, ch.Name),
								label,
								misc.skinChanger.previewFromVariant(ch)
							)
						end
					end
				end
...
function misc.skinChanger.dumpToolHandler(lp)
	local lines = {}
	lines[#lines + 1] = "=== Backpack.VM / ToolHandler ==="
...
function misc.skinChanger.dump()
	...
	if tool then
		...
		for _, l in ipairs(misc.skinChanger.dumpNamedHits(tool, { "GSight", "GLaser", "WeaponHandle" }, "TOOL")) do
			lines[#lines + 1] = l
		end
	...
	for _, d in ipairs(root:GetDescendants()) do
		if d:IsA("MeshPart") and misc.skinChanger.shouldSkinMesh(d) then
			pcall(function()
				...
				local defMesh = d:GetAttribute("VG_DefMesh")
...
function misc.skinChanger.toolAlreadyHasSkin(tool, skinKey)
	...
	if misc.skinChanger.isVariantSkinKey(skinKey) then
		...
		for _, d in ipairs(tool:GetDescendants()) do
			if d:IsA("MeshPart") and misc.skinChanger.shouldSkinMesh(d) then
				any = true
				if d:GetAttribute("VG_VarSkin") ~= skinKey then
					return false
				end
			end
		end
...
function misc.skinChanger.listAllWeapons()
	...
	-- Display models = full ItemStats-ish gun list (Tommy, Tommy-S, …)
	local display = misc.skinChanger.getDisplayWepModels()
	if display then
		for _, ch in ipairs(display:GetChildren()) do
			if ch:IsA("Model") then
				addGun(ch.Name)
			end
		end
	end
	-- SkinVariants mesh skins (mare_trickshot → Mare)
	local variants = misc.skinChanger.getSkinVariants()
	if variants then
		for _, catName in ipairs({ "Guns", "Melees" }) do
			local folder = variants:FindFirstChild(catName)
			if folder then
				for _, ch in ipairs(folder:GetChildren()) do
					if ch:IsA("Model") then
						local g = misc.skinChanger.parseVariantModelName(ch.Name)
						if g then
							addGun(g)
						end
					end
	...
	-- Invalidate weaponsByLen cache when list rebuilds
	misc.skinChanger._wepCache = nil
	table.sort(out)
	return out
end
...
function misc.skinChanger.skinLabel(fullName)
	...
	if misc.skinChanger.isVariantSkinKey(fullName) then
		local _, modelName = misc.skinChanger.variantPathFromKey(fullName)
		local _, label = misc.skinChanger.parseVariantModelName(modelName or "")
		return label or modelName or "?"
	end
...
function misc.skinChanger.applyNamed(gunName, skinKey)
	...
	-- Mesh variants (Storage.SkinVariants)
	if misc.skinChanger.isVariantSkinKey(skinKey) then
		if not misc.skinChanger.findVariantModel(skinKey) then
			return false, "variant missing: " .. tostring(skinKey)
		end
		local n = 0
		local seen = {}
		local function tryOnce(tool)
			...
			seen[tool] = true
			n = n + misc.skinChanger.applyVariantToTool(tool, skinKey, true)
		end
		local function tryContainer(container)
			if not container then
				return
		...
		if S and S.CrimSkinDropped then
			n = n + (misc.skinChanger.applyDroppedForGunVariant(gunName, skinKey) or 0)
		end
	...
	local tmpl = nil
	local variantKeyFromMesh = nil
	if texId then
		-- Official MeshVariant → SkinVariants / RepPBR (safe; avoids blind MeshId swap)
		local label = misc.skinChanger.skinLabel(skinKey)
		local meshVar = misc.skinChanger.lookupMeshVariant(gunName, label)
	...
	-- Tex skin with MeshVariant but no RepPBR: apply TextureID + safe variant (no melee MeshId)
	if texId and variantKeyFromMesh and misc.skinChanger.findVariantModel(variantKeyFromMesh) then
		local n = 0
		local seen = {}
		local function tryOnce(tool)
			if not tool or seen[tool] then
			...
			n = n + misc.skinChanger.applyVariantToTool(tool, variantKeyFromMesh, true)
		end
		local function tryContainer(container)
			if not container then
				return
		...
		if S and S.CrimSkinDropped then
			...
			n = n + (misc.skinChanger.applyDroppedForGunVariant(gunName, variantKeyFromMesh) or 0)
		end
		return true, string.format("%s → %s +mesh (%d)", misc.skinChanger.baseGunName(gunName), misc.skinChanger.skinLabel(skinKey), n)
	end
	...
	local function tryOnce(tool)
		...
		-- After RepPBR/tex, still tag MeshVariant path if we only got SA (guns may want mesh later — melee stays SA-only via applyVariantToTool safe path skipped when tmpl set)
	end
	local function tryContainer(container)
		if not container then
			return
...
function misc.skinChanger.applyDroppedForGunVariant(gunName, skinKey)
	local filter = workspace:FindFirstChild("Filter")
	local folder = filter and filter:FindFirstChild("SpawnedTools")
	local variant = misc.skinChanger.findVariantModel(skinKey)
	if not folder or not gunName or not variant then
		return 0
	end
	local S = _G.__VG_S
	local n = 0
	for _, model in ipairs(folder:GetChildren()) do
		if model:IsA("Model") and misc.skinChanger.dropNearLocal(model, S) then
			...
			if resolved and misc.skinChanger.gunsShareSkin(resolved, gunName) then
				...
				local applied = misc.skinChanger.applyVariantToInstance(model, variant, skinKey, true)
				n = n + applied
				if applied > 0 then
					pcall(function()
						model:SetAttribute("VG_DropSkin", skinKey)
...
function misc.skinChanger.applySavedForTool(tool, quiet)
	...
	if misc.skinChanger.toolAlreadyHasSkin(tool, key) then
		...
		if vmTool and not misc.skinChanger.toolAlreadyHasSkin(vmTool, key) then
			if misc.skinChanger.isVariantSkinKey(key) then
				misc.skinChanger.applyVariantToInstance(vmTool, misc.skinChanger.findVariantModel(key), key, true)
			elseif misc.skinChanger.isTexSkinKey(key) then
	...
	if misc.skinChanger.isVariantSkinKey(key) then
		misc.skinChanger.applyVariantToTool(tool, key, quiet ~= false)
...
function misc.skinChanger.cycleForCurrent()
	...
	for _, row in ipairs(misc.skinChanger.listTexSkinsForGun(tool.Name)) do
		if not saLabels[string.lower(row.label)] then
			...
			saLabels[string.lower(row.label)] = true
		end
	end
	for _, row in ipairs(misc.skinChanger.listVariantSkinsForGun(tool.Name)) do
		if not saLabels[string.lower(row.label)] then
			keys[#keys + 1] = row.full
			saLabels[string.lower(row.label)] = true
		end
...
function misc.skinChanger.start()
	...
	misc.skinChanger._upgJunkCleaned = false
	task.defer(function()
		task.wait(0.25)
		misc.skinChanger.cleanupLeftoverUpgradeJunk()
	end)
	local lp = getLP()
	if not lp then
		return
...
local function startMaster(S)
	...
	master.conn = RS.Heartbeat:Connect(perfWrap("Criminality.Main", function()
		...
		-- (weapon upgrade-look feature removed)
